<?php if (Text::encode($filter->orderType) == 'material'):?>
	<?=Text::encode($filter->fio)?> заказал <?=Text::encode($filter->weightString)?> <?=Text::encode($filter->materialName)?> по <?=Text::encode($filter->cost)?> руб. за 	тонну, на общую сумму <?=Text::encode($filter->totalCost)?> руб. без учёта доставки.<br />
